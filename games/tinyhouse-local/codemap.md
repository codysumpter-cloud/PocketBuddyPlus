...
editable floor cells and wall/door edges. Editable room recipes reconstruct the
pack's Bathroom, Kitchen, Office, and Japanese showcase compositions from movable
manifest assets. Optional showcase GIF/PNG previews remain local object URLs.
Authored furniture direction metadata provides four-way rotation where the pack
contains real views and an honest mirror-only fallback where it does not. The
camera keeps the fixed pixel-isometric perspective while adding cursor-centered
zoom, empty-canvas pan, touch pinch, damping, and momentum. Cozy Mode adds local
productivity, ambience, room reactions, and a narrow Pocket Buddy+ host bridge.

## Files

- `index.html` — game shell, catalog, room layers, structure controls, Blueprint planner, room-template gallery, rotation controls, camera controls, and Cozy entry point.
...
- `app.js` — original exact isometric transforms, furnishing placement, animation state, selection, camera compatibility, and legacy two-way flip behavior intercepted by the rotation plugin.
...
- `room-templates-core.js` — metadata-only Bathroom, Kitchen, Office, and Japanese room recipes, exact licensed manifest IDs, preview-name matching, and missing-asset validation.
- `rotation-core.js` — pure directional-family discovery, A/B/C/D ordering, front/back pairing, safe mirror completion, known multi-view sheet metadata, and rotation-index normalization.
...
- `house-grid-render.js` — dynamic floor/wall/door rendering, base camera fit, and full-house PNG export.
...
- `room-templates.js` — Rooms gallery, editable template application, complete-house backup/restore, whole-room animation trigger, explicit UUID preview assignment, named-preview matching, and ephemeral preview URL lifecycle.
- `four-way-rotation.js` — local spritesheet slicing, authored visual selection, R/Shift+R controls, inspector metadata, persistent rotation state, DOM reconciliation after legacy renders, and rotation-aware full-house export.
- `diorama-camera.js` — fixed-isometric camera controller with pointer-anchored zoom, empty-canvas/Space/middle pan, touch pinch, damping, momentum, reduced-motion handling, and smooth house fit.
...
- `test-rotation-camera-contract.mjs` — real-manifest four-way family resolution, sheet slicing rules, honest two-way fallback, persisted/exported rotations, script ordering, cursor zoom, touch pinch, damping, momentum, and fixed-perspective boundary.
...
- Room templates store only manifest IDs, coordinates, scale, wall anchors, support relationships, animation metadata, and rotation indices; they contain no purchased image bytes.
...
- Four-way rotation uses authored A/B/C/D assets, front/back pairs, base/B pairs, or declared sheet slices; genuinely single-view art remains two-way rather than receiving a fake CSS quarter-turn.
- Rotation indices persist in the existing placement serialization and are preserved by complete-house save/load, template backup/restore, duplication, drag, and export.
- The camera remains fixed-isometric 2D. Continuous orbit is explicitly unsupported because it would expose perspectives absent from the source pixel art.
