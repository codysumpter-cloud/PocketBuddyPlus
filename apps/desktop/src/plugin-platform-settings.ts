...
export type PluginAiProviderKind = "none" | "anthropic" | "openai" | "nvidia" | "ollama";
...
function normalizeSettings(value: unknown): PluginPlatformSettings {
  ...
  const provider = ["none", "anthropic", "openai", "nvidia", "ollama"].includes(String(ai.provider)) ? String(ai.provider) as PluginAiProviderKind : "none";
  return {
