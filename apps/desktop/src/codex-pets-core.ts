...
export const maxCodexAnimationManifestBytes = 2 * 1024 * 1024;
...
export interface CodexPetMetadata {
  ...
  readonly animationManifestPath?: "animation-manifest.json";
}
...
export function validateCodexPetMetadata(value: unknown, folderName: string): CodexPetMetadata {
  ...
  if (value.animationManifestPath !== undefined && value.animationManifestPath !== "animation-manifest.json") throw new Error("Codex pet animationManifestPath must be animation-manifest.json.");
  return {
    id: value.id,
