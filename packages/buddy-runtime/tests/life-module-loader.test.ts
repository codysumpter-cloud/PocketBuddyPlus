...
import {
  ...
  LIFE_MODULE_PATH_ENV,
  loadBundledLifeModule,
  loadLifeModule,
} from "../src/life-module-loader.js";

const here = dirname(fileURLToPath(import.meta.url));
// tests compile to dist-tests/tests, so fixtures live beside the source tree.
const fixture = (name: string) => pathToFileURL(join(here, "..", "..", "tests", "fixtures", name)).href;

const BUNDLED = pathToFileURL(join(here, "..", "..", "..", "buddy-life-lgpl", "dist", "reference-module.js")).href;
const base = { bundledSpecifier: BUNDLED, env: {} as Record<string, string | undefined> };
...
test("a failed override does NOT silently fall back", async () => {
  ...
  if (!failed.ok) assert.equal(failed.origin, "override");

  const recovered = await loadBundledLifeModule(base);
  assert.equal(recovered.ok, true);
  if (recovered.ok) assert.equal(recovered.origin, "bundled");
});
...
test("the override is read only from the injected environment", async () => {
  ...
  assert.equal(result.ok, true);
  if (result.ok) assert.equal(result.origin, "bundled");
});
...
test("bundled module advances deterministically regardless of call cadence", async () => {
  ...
  for (let i = 0; i < 60; i += 1) b.module.advance({ elapsedSeconds: 1 });
