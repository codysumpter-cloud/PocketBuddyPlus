 ...
 */
import {
  LIFE_MODULE_API_VERSION,
  validateLifeModule,
  type BuddyLifeModule,
} from "@open-pets/buddy-life-lgpl";
...
export type LifeModuleOrigin = "bundled" | "override";

export interface LifeModuleLoadSuccess {
  readonly ok: true;
  readonly module: BuddyLifeModule;
  readonly origin: LifeModuleOrigin;
  /** Absolute specifier actually loaded, for diagnostics. */
  readonly specifier: string;
  readonly diagnostics: LifeModuleDiagnostics;
}

export interface LifeModuleLoadFailure {
  readonly ok: false;
  readonly origin: LifeModuleOrigin;
  readonly specifier: string;
  readonly code:
...
export type LifeModuleLoadResult = LifeModuleLoadSuccess | LifeModuleLoadFailure;

export interface LifeModuleDiagnostics {
  readonly origin: LifeModuleOrigin;
  readonly specifier: string;
...
export interface LoadLifeModuleOptions {
  /**
   * Environment to read the override from. Injected so tests never mutate the
   * real process environment.
   */
  readonly env?: Readonly<Record<string, string | undefined>>;
  readonly timeoutMs?: number;
  /** Overridden in tests; production always resolves the bundled package. */
  readonly bundledSpecifier?: string;
...
function diagnosticsFor(module: BuddyLifeModule, origin: LifeModuleOrigin, specifier: string): LifeModuleDiagnostics {
...
async function loadFrom(
  specifier: string,
  origin: LifeModuleOrigin,
  options: LoadLifeModuleOptions,
): Promise<LifeModuleLoadResult> {
  const timeoutMs = options.timeoutMs ?? DEFAULT_LOAD_TIMEOUT_MS;
  const importModule = options.importModule ?? ((target: string) => import(target));
  ...
  const factory = (namespace as Record<string, unknown>).createLifeModule;
  ...
  try {
    const produced = await withTimeout(Promise.resolve((factory as () => unknown)()), timeoutMs);
    if (produced === TIMEOUT) {
  ...
  const module = candidate as BuddyLifeModule;
 ...
 */
export async function loadLifeModule(options: LoadLifeModuleOptions = {}): Promise<LifeModuleLoadResult> {
  const env = options.env ?? process.env;
  const bundled = options.bundledSpecifier ?? BUNDLED_SPECIFIER;
  const override = env[LIFE_MODULE_PATH_ENV]?.trim();

  if (override) return loadFrom(override, "override", options);
  return loadFrom(bundled, "bundled", options);
}

/** Explicit recovery path after a failed override. Never automatic. */
export async function loadBundledLifeModule(options: LoadLifeModuleOptions = {}): Promise<LifeModuleLoadResult> {
