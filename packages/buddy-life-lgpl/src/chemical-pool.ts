...
export class BuddyBiologyChemicalPool {
  ...
  private readonly concentrations = new Float32Array(CHEMICAL_COUNT);
  /** Encoded half-life rates, 0..255. */
  private readonly halfLives = new Uint8Array(CHEMICAL_COUNT);
  ...
  concentration(chemicalId: number): number {
    if (!BuddyBiologyChemicalPool.validId(chemicalId)) return 0;
    return this.concentrations[chemicalId];
  }

  setConcentration(chemicalId: number, value: number): void {
    if (!BuddyBiologyChemicalPool.validId(chemicalId)) return;
    this.concentrations[chemicalId] = clamp01(value);
  }

  adjust(chemicalId: number, delta: number): void {
    this.setConcentration(chemicalId, this.concentration(chemicalId) + delta);
  }

  halfLife(chemicalId: number): number {
    if (!BuddyBiologyChemicalPool.validId(chemicalId)) return 0;
    return this.halfLives[chemicalId];
  }

  setHalfLife(chemicalId: number, encodedRate: number): void {
    if (!BuddyBiologyChemicalPool.validId(chemicalId)) return;
  ...
  toData(): { readonly concentrations: number[]; readonly half_lives: number[] } {
    return {
