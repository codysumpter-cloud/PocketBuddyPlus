...
import {
  ...
  PUNISHMENT_ID,
  REWARD_ID,
} from "./chemical-pool.js";
import {
  LIFE_MODULE_API_VERSION,
  type AdvanceRequest,
  type BuddyLifeModule,
  type CreateCreatureRequest,
  type HostOutcomeRequest,
  type LifeEvent,
  type LifeModuleHealth,
  type LifeModuleIdentity,
  type LifeSnapshot,
  type StimulusRequest,
} from "./contract.js";
...
export const referenceIdentity: LifeModuleIdentity = {
...
export class ReferenceBuddyLifeModule implements BuddyLifeModule {
  readonly identity = referenceIdentity;

  private chemicals = new BuddyBiologyChemicalPool();
  private creatureId = "";
  ...
  applyStimulus(request: StimulusRequest): { snapshot: LifeSnapshot; events: readonly LifeEvent[] } {
    this.requireInitialized();
    if (!request || typeof request.kind !== "string") throw new Error("applyStimulus requires a kind");
  ...
  submitOutcome(request: HostOutcomeRequest): { snapshot: LifeSnapshot; events: readonly LifeEvent[] } {
    this.requireInitialized();
    if (!request || typeof request.action !== "string") throw new Error("submitOutcome requires an action");
    ...
    if (reward > 0) this.chemicals.adjust(REWARD_ID, reward);
    if (punishment > 0) this.chemicals.adjust(PUNISHMENT_ID, punishment);
    return {
  ...
  private requireInitialized(): void {
    if (!this.initialized) throw new Error("life module is not initialized; call createCreature or loadCreature");
  }
}

/** Factory the host looks for when loading a module. */
export function createLifeModule(): BuddyLifeModule {
