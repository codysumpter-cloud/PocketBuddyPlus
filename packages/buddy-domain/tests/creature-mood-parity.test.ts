...
test("action memory keeps a FIFO window but counts every occurrence", () => {
  const state = new BuddyCreatureState(0, "x");
  for (let i = 0; i < 12; i += 1) state.rememberAction(`act_${i % 3}`);
...
test("working memory is bounded and stores copies", () => {
  ...
  entry.note = "mutated";
  assert.equal((state.workingMemory[0] as { note: string }).note, "a", "must store a copy");
  for (let i = 0; i < 20; i += 1) state.addWorkingMemory({ i });
