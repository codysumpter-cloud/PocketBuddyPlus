...
export const DEFAULT_RELATIONSHIP: Readonly<Record<string, number>> = Object.freeze({
  affection: 0.5,
  trust: 0.5,
  familiarity: 0.1,
  respect: 0.4,
...
export const DEFAULT_STATS: Readonly<Record<string, number>> = Object.freeze({
  level: 1,
  experience: 0,
  skill_points: 0,
  rerolls: 1,
...
export function safeBuddyId(value: string): string {
...
export class BuddyCreatureState {
  ...
  learnedAssociations: Record<string, unknown> = {};
  actionCounts: Record<string, number> = {};
  cooldownUntilUnix: Record<string, number> = {};
  lastActions: string[] = [];
  workingMemory: Record<string, unknown>[] = [];
  episodicMemoryRefs: string[] = [];
  activeTasks: string[] = [];
  inventory: Record<string, unknown> = {};
  customization: Record<string, unknown> = {};
  flags: Record<string, unknown> = {};
  mood: BuddyMood = { label: "content", valence: 0.5, arousal: 0.3, dominance: 0.5 };
  ...
  ensureDefaults(): void {
    ...
    for (const key of Object.keys(DEFAULT_STATS)) {
      if (!(key in this.stats)) this.stats[key] = DEFAULT_STATS[key];
    }

    const mood = (typeof this.mood === "object" && this.mood !== null ? this.mood : {}) as Partial<BuddyMood>;
    this.mood = {
  ...
  rememberAction(actionId: string, maximum = 8): void {
    if (!actionId) return;
  ...
  addWorkingMemory(entry: Record<string, unknown>, maximum = 16): void {
  ...
  static fromData(data: unknown, nowUnix = 0): BuddyCreatureState {
    const state = new BuddyCreatureState(nowUnix);
    if (typeof data !== "object" || data === null) return state;
    const record = data as Record<string, unknown>;

    const str = (key: string, fallback: string) => (typeof record[key] === "string" ? (record[key] as string) : fallback);
    const num = (key: string, fallback: number) => (typeof record[key] === "number" && Number.isFinite(record[key] as number) ? (record[key] as number) : fallback);
    const obj = (key: string) => (typeof record[key] === "object" && record[key] !== null && !Array.isArray(record[key]) ? (record[key] as Record<string, unknown>) : {});
    const arr = (key: string) => (Array.isArray(record[key]) ? (record[key] as unknown[]) : []);
    ...
    state.relationship = Object.fromEntries(
      Object.entries(obj("relationship")).filter(([, v]) => typeof v === "number") as [string, number][],
    );
    state.stats = Object.fromEntries(
      Object.entries(obj("stats")).filter(([, v]) => typeof v === "number") as [string, number][],
    );
    state.learnedAssociations = obj("learned_associations");
    state.actionCounts = Object.fromEntries(
      Object.entries(obj("action_counts")).filter(([, v]) => typeof v === "number") as [string, number][],
    );
    state.cooldownUntilUnix = Object.fromEntries(
      Object.entries(obj("cooldown_until_unix")).filter(([, v]) => typeof v === "number") as [string, number][],
    );
    state.lastActions = arr("last_actions").filter((v): v is string => typeof v === "string");
    state.workingMemory = arr("working_memory").filter((v): v is Record<string, unknown> => typeof v === "object" && v !== null);
    state.episodicMemoryRefs = arr("episodic_memory_refs").filter((v): v is string => typeof v === "string");
    state.activeTasks = arr("active_tasks").filter((v): v is string => typeof v === "string");
    state.inventory = obj("inventory");
    state.customization = obj("customization");
    state.flags = obj("flags");
    if (typeof record.mood === "object" && record.mood !== null) state.mood = record.mood as BuddyMood;
