...
/**
 * Where the room is saved. In the app this was browser-local storage; a plugin
 * panel is a sandboxed `file:` page whose storage is neither shared with the
 * app nor durable, so the host owns the save and hands it in at mount.
 */
export interface HomeStateStore {
...
class PhaserHomeScene extends Scene {
  ...
  private drawPlayer(graphics: Graphics, origin: Vector2): void {
  ...
  private roomOrigin(): Vector2 {
    ...
    return new Vector2(x, Math.max(150, this.scale.height * 0.24));
  }
